...
(defparameter +col-labels+
  (mapcar (lambda (n)
            (format nil "~a" n))
          (alexandria:iota +cols+ :start 1)))

(defparameter +row-labels+
  (mapcar (lambda (c)
            (format nil "~a" (code-char c)))
          (alexandria:iota +rows+ :start (char-code #\A))))
...
(defmethod (setf checker) :before ((val null) (field field))
  (alexandria:when-let ((checker (checker field)))
...
(defun switch-player (game-state)
  ...
  (ecase (player game-state)
    (:player-1 (setf (player game-state) :player-2))
    (:player-2 (setf (player game-state) :player-1))))
...
(defun get-field (game-state row col)
  (unless (integerp row)
    (setf row (position row "ABCDEFGH" :test #'string=)))
...
(defun takes-checker-p (game-state)
  ...
  (dotimes (row +rows+)
    (dotimes (col +cols+)
      (alexandria:when-let* ((field (get-field game-state row col))
...
(define-presentation-method present
  ...
  (alexandria:when-let* ((field (field checker))
                         (x (+ (col field) .5))
                         (y (+ (row field) .5))
